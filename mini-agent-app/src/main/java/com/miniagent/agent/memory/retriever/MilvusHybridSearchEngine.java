...
public class MilvusHybridSearchEngine implements HybridSearchEngine {
        ...
        try {
            ...
            for (SearchResp.SearchResult hit : groups.get(0)) {
                ...
                if (ent instanceof Map<?, ?> map) {
                    ...
                    if (entity.getStatus() != AgentMemoryEntryEntity.Status.ACTIVE) {
                        continue;
                    }
                    // Milvus is an accelerator, not an authorization boundary. The
                    // relational row must still match the complete requested scope.
                    MemoryEntry candidate = MemoryEntryMapper.fromEntity(entity);
                    if (!matchesScope(query, entity)) {
                        continue;
                    }

                    MemoryEntry memory = candidate;
    ...
    /** Exact post-filter used after vector lookup to prevent cross-scope leakage. */
    static boolean matchesScope(MemoryQuery query, AgentMemoryEntryEntity entity) {
        if (query == null || query.getScope() == null || entity == null) {
            return false;
        }
        MemoryScope requested = query.getScope();
        return Objects.equals(requested.tenantId(), entity.getTenantId())
                && entity.getScopeType() != null
                && requested.scopeType() != null
                && requested.scopeType().name().equals(entity.getScopeType().name())
                && Objects.equals(requested.scopeId(), entity.getScopeId());
    }
    ...
    public boolean upsert(com.miniagent.memory.model.MemoryEntry memory) {
        ...
        try {
            if (memory == null || memory.getId() == null || memory.getContent() == null) {
                return false;
            }
