...
public class GoalCompiler {
    ...
    private static final String COMPILER_SYSTEM = """
            ...
            规则：至少 3 个节点；依赖无环；每节点必须有 doneWhen；id 唯一。
            """;
    ...
    boolean validate(TaskGraph g, TaskPlan plan) {
        ...
        if (needRich && g.nodes().size() < 3) return false;
    ...
    TaskGraph templateGraph(String userMessage, TaskPlan plan) {
        ...
        while (nodes.size() < 3) {
            int i = nodes.size() + 1;
            String id = "n" + i;
            List<String> deps = nodes.isEmpty() ? List.of() : List.of(nodes.get(nodes.size() - 1).id());
            ...
            String done = i == 3 ? "note_required" : "note_required";
