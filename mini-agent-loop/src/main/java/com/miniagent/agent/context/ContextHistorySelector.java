...
public class ContextHistorySelector {
    ...
    private List<Scored> scoreByVector(List<ChatMessage> pool, int from, String query) {
        ...
        if (qVec.length == 0) {
            throw new IllegalStateException("query embedding empty");
        }
        ...
        for (int i = 0; i < pool.size(); i++) {
            if (StringUtils.isBlank(texts.get(i))) {
                continue;
            }
    ...
    private List<ChatMessage> pick(
        ...
        for (Scored s : scored) {
            if (s.score() < minScore && !picked.isEmpty()) {
                break;
            }
            picked.add(s.index());
            if (picked.size() >= topK) {
