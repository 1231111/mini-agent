...
public interface MemoryManager {
    ...
    /**
     * 按租户读取一条记忆。租户是数据访问契约的一部分，不能只按全局自增 ID 查询。
     */
    Optional<MemoryEntry> getMemory(String tenantId, Long id);

    /**
     * 按租户更新一条记忆。保留旧签名供内部兼容，但 Web/API 层应始终使用该方法。
     */
    void updateMemory(String tenantId, Long id, MemoryEntry update);

    /** 删除一条记忆（软删除） */
    void deleteMemory(Long id);

    /** 按租户软删除，供所有外部 API 使用。 */
    void deleteMemory(String tenantId, Long id);
    ...
    /** 普通用户回忆时同时约束 userId，避免同租户横向读取。 */
    List<Episode> recallEpisodes(String query, String tenantId, String userId,
