...
public class ToolRouter {
    ...
    private int score(TaskNode node, String tool) {
        ...
        if (cap.contains("write") && t.contains("write")) {
            s += SCORE_CAP_MATCH;
        }
        if (cap.contains("web") && (t.contains("web") || t.contains("http"))) {
            s += SCORE_CAP_MATCH;
        }
        if (cap.contains("image") && (t.contains("image") || t.contains("comfy"))) {
            s += SCORE_CAP_MATCH;
        }
        if (cap.contains("code") && (t.contains("code") || t.contains("search") || t.contains("ast")))
            s += SCORE_CAP_MATCH;
        if (name.contains("写") && t.contains("write")) {
            s += SCORE_NAME_MATCH;
        }
        if (name.contains("搜索") && t.contains("search")) {
            s += SCORE_NAME_MATCH;
        }
